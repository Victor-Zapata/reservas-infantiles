...
export default function AdultoPage() {
  ...
  const handleChange = <K extends keyof DatosAdulto>(campo: K, valor: DatosAdulto[K]) => {
    setAdulto((prev) => ({ ...prev, [campo]: valor }));
  };

  const handleContactoChange = <K extends keyof Contacto>(campo: K, valor: Contacto[K]) => {
    setAdulto((prev) => ({
      ...prev,
